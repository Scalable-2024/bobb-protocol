...
def create_handshake_message(name, device_function, public_key, port, ip):
    ...
    handshake_body = SatelliteHandshake(
        name, device_function, public_key, port, connected_nodes).build_message()
...
def write_received_handshake(handshake_data, bobb_header):
    ...
    if write_to_json(source_ip, device_function, public_key, source_port, connected_nodes):
        print(f"Neighbour {source_ip}:{source_port} added")
    else:
        print(f"{source_ip}:{source_port} was already a neighbour, not adding again")


def write_to_json(source_ip, device_function, public_key, port, connected_nodes):
