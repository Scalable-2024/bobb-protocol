...
def send_handshake(neighbour):
    ...
    handshake_body = SatelliteHandshake(satellite_function, public_key, ip, port, connected_nodes).build_message()
    header = BobbHeaders(message_type=1, source_ipv6=ip, source_port=port)

    resp = requests.post(f"https://{n_ip}:{n_port}/handshake", verify=False, timeout=3, proxies=proxies, headers=header, json=handshake_body)
